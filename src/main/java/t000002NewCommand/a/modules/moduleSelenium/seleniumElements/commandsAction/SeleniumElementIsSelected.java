...
public class SeleniumElementIsSelected extends CommandSeleniumElement {

    private static Logger logger = LoggerFactory.getLogger(SeleniumElementIsSelected.class);

    public SeleniumElementIsSelected(IAppGetableDriver appSeleniumElement, ISeleniumElement seleniumElement) {
        super(appSeleniumElement, seleniumElement);
    }

    @Override
    public <T> T execute() {
        logger.debug("SeleniumElementIsSelected <T> T execute() Boolean");
        Expression<T> expression = (webElement) -> (T)(Boolean)webElement.isSelected();
