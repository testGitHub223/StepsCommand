...
public class SeleniumElementGetRect extends CommandSeleniumElement {

    private static Logger logger = LoggerFactory.getLogger(SeleniumElementGetRect.class);

    public SeleniumElementGetRect(IAppGetableDriver appSeleniumElement, ISeleniumElement seleniumElement) {
        super(appSeleniumElement, seleniumElement);
    }

    @Override
    public <T> T execute() {
        logger.debug("SeleniumElementGetRect <T> T execute() Rectangle");
        Expression<T> expression = (webElement) -> (T)webElement.getRect();
