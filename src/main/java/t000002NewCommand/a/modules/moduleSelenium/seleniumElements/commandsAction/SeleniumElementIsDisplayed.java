...
public class SeleniumElementIsDisplayed extends CommandSeleniumElement {

    private static Logger logger = LoggerFactory.getLogger(SeleniumElementIsDisplayed.class);

    public SeleniumElementIsDisplayed(IAppGetableDriver appSeleniumElement, ISeleniumElement seleniumElement) {
        super(appSeleniumElement, seleniumElement);
    }

    @Override
    public <T> T execute() {
        logger.debug("SeleniumElementIsDisplayed <T> T execute() Boolean");
        Expression<T> expression = (webElement) -> (T)(Boolean)webElement.isDisplayed();
